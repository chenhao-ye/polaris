...
print 'alg threads txn_len num_hs pos_hs synthetic zipfian read_ratio specified flip throughput debug2 debug8 debug9 debug10 debug11 time_wait time_abort run_time abort_cnt txn_cnt'
    ...
    for each_file in os.listdir(check_dir + '/' + subdir):
       if each_file == 'config.json':
           with open(check_dir + '/' + subdir + '/' + each_file, 'r')  as config_file:
               ...
               res1 = dict_from_file['alg'] + ' ' + str(dict_from_file['threads']) + ' ' + str(dict_from_file['txn_len']) + ' ' + str(dict_from_file['num_hs']) + ' ' + dict_from_file['pos_hs'] + ' ' +  str(dict_from_file['synthetic']) + ' ' + str(dict_from_file['zipfian']) + ' ' + str(dict_from_file['read_ratio']) + ' ' + str(dict_from_file['specified']) + ' ' + str(dict_from_file['flip']) + ' '
       ...
       if each_file == 'stats.json':
           with open(check_dir + '/' + subdir + '/' + each_file, 'r')  as output_file:
               ...
               res2 = dict_from_file['throughput'] + ' ' + dict_from_file['debug2'] + ' ' + dict_from_file['debug8'] + ' ' + dict_from_file['debug9'] + ' ' + dict_from_file['debug10'] + ' ' + dict_from_file['debug11'] + ' ' + dict_from_file['time_wait'] + ' ' + dict_from_file['time_abort'] + ' ' + dict_from_file['run_time'] + ' ' + dict_from_file['abort_cnt'] + ' ' + dict_from_file['txn_cnt'] + ' '
