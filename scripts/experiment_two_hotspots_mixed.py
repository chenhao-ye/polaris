...
class Experiment(object):
    def __init__(self):
        ...
        self.exp_name = 'two_hotspots_mixed'
        ...
        config = {
          ...
          'txn_len': [4, 16],    # number of requests in the txn
          #'num_hs' : [1], # 0, 1, 2
          'num_hs' : [2],
          #'pos_hs' : ['TOP', 'MID', 'BOT'], # this is only for one hotspot
          'pos_hs' : ['SPECIFIED'],
          'specified': [0.1, 0.5, 0.9],  # the position of the second hotspot
        ...
        with open('/users/kanwu/DBx1000-Private/scripts/synthetic_debug.sh.template', 'r') as input_file, open('/users/kanwu/DBx1000-Private/synthetic_debug.sh', 'w') as output_file:
            for line in input_file:
                ...
                if 'specified=' in line:
                    output_file.write('specified=' + str(config['specified']) + '\n')
                    continue
                if 'flip=' in line:
                    output_file.write('flip=' + str(config['flip']) + '\n')
                    continue
