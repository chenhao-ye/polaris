...
def compile(job):
    ...
    if ret != 0:
        print("ERROR in compiling, output saved in temp.out", file=sys.stderr)
        exit(0)
    else:
...
if __name__ == "__main__":
    print("usage: path/to/json [more args]", file=sys.stderr)
    fname = sys.argv[1]
    idx = 2
    if ".json" not in fname:
        fname = "experiments/default.json"
        idx = 1
    print("- read config from file: {}".format(fname), file=sys.stderr)
    ...
    if not eval_arg(job, "EXEC_ONLY"):
        ...
        set_ndebug(ndebug)
        if ndebug:
            print("- disable assert()", file=sys.stderr)
        compile(job)
    numa = eval_arg(job, "UNSET_NUMA") == False
    if not numa:
        print("- disable interleaving allocation across numa nodes", file=sys.stderr)
