...
for thd in 16 32
do
		python test.py experiments/synthetic_ycsb.json FLIP_RATIO=0.5 FIXED_HS=${fix} NUM_HS=2 THREAD_CNT=${thd} SPECIFIED_RATIO=${pos} CC_ALG=${alg} OUTPUT_TO_FILE=true CPU_FREQ=2.8
done
done
done
done
